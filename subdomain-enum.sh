...
perform_recon() {
  ...
  subfinder -d "$domain" -o "$domain_dir/subfinder.txt" || { echo "Error: Subfinder encountered an error."; continue; }

  # Run amass in the background
  echo "Running amass in the background..."
  amass enum -passive -d "$domain" -o "$domain_dir/amass.txt" || { echo "Error: Amass encountered an error."; continue; }
  ...
  httprobe -c 50 -t 3000 -p http,https < "$combined_file" | tee "$domain_dir/3-subd$domain.txt" || { echo "Error: Httprobe encountered an error."; continue; }
